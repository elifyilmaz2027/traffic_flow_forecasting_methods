...
def armodel(train_data, test_data):
    ...
    pd.DataFrame(train_predictions).to_csv("point_forecasts/ar_pems_716933_train.csv")
    pd.DataFrame(test_predictions).to_csv("point_forecasts/ar_pems_716933_test.csv")
...
def armamodel(train_data, test_data):
    ...
    pd.DataFrame(train_predictions).to_csv("point_forecasts/arma_pems_716933_train.csv")
    pd.DataFrame(test_predictions).to_csv("point_forecasts/arma_pems_716933_test.csv")
    return train_predictions, test_predictions


def arimamodel(train_data, test_data):
    arima = ARIMA(train_data, order=(4,1,0))
    arima_fit = arima.fit()    
    print(arima_fit.summary())
    ...
    pd.DataFrame(train_predictions2).to_csv("point_forecasts/arima_pems_716933_train.csv")
    pd.DataFrame(test_predictions2).to_csv("point_forecasts/arima_pems_716933_test.csv")
...
def sarimamodel(data):
    ...
    train_predictions = sarima_fit.predict(start=len(train_y),end=len(train_y)+len(train_y)-1,dynamic=train_y.all(),exog=train_X)
    train_predictions2 = []
    for t in range(len(train_y)):
        output_train = train_predictions[t] + train_y[t]
        train_predictions2.append(output_train)
    
    test_predictions = sarima_fit.predict(start=len(train_y),end=len(train_y)+len(test_y)-1,dynamic=test_y.all(),exog=test_X)
    test_predictions2 = []
    test_y2=[]
    ...
    pd.DataFrame(train_predictions2).to_csv("point_forecasts/sarima_pems_716933_train.csv")
    pd.DataFrame(test_predictions2).to_csv("point_forecasts/sarima_pems_716933_test.csv")
    return train_predictions2, test_predictions2
